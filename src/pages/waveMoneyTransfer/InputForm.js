...
const InputForm = ({ editData }) => {
  ...
  const handleSubmit = useCallback(
    async (values) => {
      editData
        ? await WaveMoneyTransferService.update(values, editData?.id)
        : await WaveMoneyTransferService.store(values);

      reset();
      navigate("/admin/list-wave-money-transfer");
    },
    [editData, reset]
  );
